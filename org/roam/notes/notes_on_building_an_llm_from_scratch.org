...
*** Tokenizing Text 
**** special tokens like "unk "are used to indicate words that aren't in the vocab 
**** special tokens like "endoftext" are used to indicate the beginning of a new text source when multiple sources are used
